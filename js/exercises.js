...
//   jump:  counts towards jump / ground-contact volume (js/analytics.js)
//   contacts: ground contacts per rep, when a rep is not a single contact
//             (default 1). "each side" items are doubled by the item's own
//             side flag, not here.
// ============================================================================
...
export const EX = {
  ...
  ol_speed_drop:     { name: 'One Leg Speed Drop Stick', jump: true, mux: '5QTKLKPEknTlbppgKJUBYR3FzynKG4014RfB2orXFoPY', yq: 'single leg drop stick landing drill' },
  rapid_decel:       { name: 'Rapid Deceleration', jump: true, mux: 'T02jtc200dB6I3fbVnn5l7viDxOuLRj87dmlG6oPojqfM', yq: 'rapid deceleration drill jumping' },
  lat_bounds:        { name: 'Lateral Bounds', jump: true, mux: 'ax3KRxnL201e01C533abD3VH9aZU9fPnXAC7JWXr02R3aY' },
  standing_jump:     { name: 'Standing Jump', jump: true, mux: 'De2602Ug01dfpFev8Q02k5X02UjUytNuaaP02TNTvhygaMes', yq: 'standing vertical jump technique' },
  ...
  // --- Phase 2 plyo ---
  sd_lat_push:       { name: 'Speed Drop Lateral Push', jump: true, mux: 'Sw02cNwOfhYi8gpNkaDXbZ8YpHQnzgwFQSXGIUhlyjE4', yq: 'speed drop lateral push jump drill' },
  angle_bounds:      { name: 'Angle Bounds', jump: true, mux: 'O2UlESm6IwmEadIHM5pCJg001omN46rRkCRJ01qMDDIuo', yq: 'angled bounds plyometric' },
  penult_jumps:      { name: 'Penultimate Jumps', jump: true, contacts: 2, mux: 'zMiBBjABiYk00G02mSHPovc01e6pYC01Gh8AWcXdhTT4kBk', yq: 'penultimate step jump volleyball' },
  tf_max_touch:      { name: 'Two Foot Max Approach Touch', jump: true, mux: 'xUcch00qzk8kP6dkZBz7BD7FbZOVNJLio7tQ7gv3Hyi8', yq: 'two foot approach jump max touch volleyball' },
  power_skip:        { name: 'Power Skip', jump: true, mux: 'jV5mGMC01ARFoqsMERt5KA9Kin00f8RF7i71mxPsdE4wU' },
  ...
  // --- Phase 3 plyo ---
  sd_of_jump:        { name: 'Speed Drop One Foot Jump', jump: true, mux: 'sScjUtISVkm5Qq577PFvhYoTaoSfjrRoyZh9g7LiwhU', yq: 'speed drop single leg jump' },
  accel_jump:        { name: 'Acceleration Jump Start', jump: true, mux: 'q02IUuFkGZJgyMXC01je5H00FGgD00DMa84hUJ4DMj2JQ6s', yq: 'acceleration jump start drill' },
  // freestyle_jump: a timed block, so `contacts` here means per BLOCK — the
  // middle of the program's own 40–50 cap. A block you actually tapped out
  // uses the real tapped count instead.
  freestyle_jump:    { name: 'Freestyle Jumping', jump: true, contacts: 45, mux: 'ntZf1IZ7Ud02CVAxagLOZyD00yhxpmyzRJJOvZ2x01tSmA', note: 'Cap: 40–50 ground contacts per 10-min block. Once you hit the cap, stop.', yq: 'freestyle jumping practice vertical jump' },
  drach_jumps:       { name: 'Drach Jumps', jump: true, mux: 'S8Lo02kDBGZ02pqGjSAYHYY1slcxryk0001pc3UIvShK023c', yq: 'drach jumps plyometric' },
  ...
  achilles_spring:   { name: 'Achilles Spring', jump: true, mux: 'f2AV600W7M00sd95Zq7mzmpdntdLptpOBtyr95h6T1uH4', yq: 'achilles spring ankle stiffness pogo' },
  penult_hurdle:     { name: 'Penultimate Hurdle Jumps', jump: true, contacts: 2, mux: '600sBgfNFKeD64sNuRisfs9600rnmSRClB5G5rnH5GKsw', yq: 'penultimate hurdle jump volleyball' },
  of_max_approach:   { name: 'One Foot Max Approach', jump: true, mux: '00UXQxGovjc1BiYnEWcp22Y4rrdcu6Ay8aZhDyRkGuBs', yq: 'one foot approach jump volleyball' },
  // --- Phase 3 core ---
  ...
  // --- Phase 4 plyo ---
  // prog_bounding: one "rep" is a whole series (four combos, court length or
  // 12 reps per leg). The program never fixes a number, so 24 is a deliberate
  // flat estimate — it keeps week-to-week trends honest without pretending to
  // be an exact count.
  free_agility:      { name: 'Freestyle Agility', mux: 'hqrU1zRQhUGuDpQM6lKTWOoJLutK4PuQQpGr6wp017BY', yq: 'freestyle agility footwork drill' },
  lb_of_jump:        { name: 'Lateral Bound One Foot Jump', jump: true, contacts: 2, mux: 'z1EWY5POXKXs01GXsaxx00f178jCgR5iXptY85sTkhzsA', yq: 'lateral bound into single leg jump' },
  consec_hurdle:     { name: 'Consecutive Hurdle Jump', jump: true, contacts: 3, mux: 's9h2rsTVogvN4fldjwn7wJRPemI00INSiqpdmzoZ8rlU' },
  prog_bounding:     { name: 'Progressive Bounding Series', jump: true, contacts: 24, mux: 'oGcoOww00T85e1xc9Y25FYqhNH01yXndt7gRn9OK00yUhY', yq: 'progressive bounding series plyometric' },
  ot_drop_jump:      { name: 'One Two Drop Jump', jump: true, contacts: 2, mux: 'Q6zAd2WMR502fXFRv7Uja6HiX00yZ2wIBX4PG2F3cUYzA', yq: 'one two drop jump plyometric' },
  versatile_jump:    { name: 'Versatile Jump Drill', jump: true, mux: 'WenHJgAjL007dPecmwmNVg0080202o397bFYZ00B5O024KrFA', yq: 'versatile jump drill multi directional' },
